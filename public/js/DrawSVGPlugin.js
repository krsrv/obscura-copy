...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() {
    ...
    function i(b) {
        ...
        if ("path" === c) {
            i = b.style.strokeDasharray, b.style.strokeDasharray = "none", d = b.getTotalLength() || 0;
            try {
                e = b.getBBox()
            } catch (m) {}
            b.style.strokeDasharray = i
        } else if ("rect" === c) d = 2 * b.getAttribute("width") + 2 * b.getAttribute("height");
            ...
            for (f = b.getAttribute("points").split(", ").join(",").split(" "), d = 0, i = f[0].split(","), "" === f[f.length - 1] && f.pop(), "polygon" === c && (f.push(f[0]), -1 === f[0].indexOf(",") && f.push(f[1])), j = 1; j < f.length; j++) h = f[j].split(","), 1 === h.length && (h[1] = f[j++]), 2 === h.length && (d += a(i[0], i[1], h[0], h[1]) || 0, i = h);
